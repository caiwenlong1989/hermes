...
<form id="dataForm" method="post" action="#" class="form-horizontal">
			...
			<#list labels as label>
				<div class="panel panel-default">
					<div class="panel-heading clearfix">
				      <h5 class="control-label pull-left"><input type="checkbox" name="label" id="chk${label.id}" value="${label.id}" />&nbsp;&nbsp;${label.name}	</h5>
	...
	<input id="labels" name="labels" type="hidden"></input>	
		...
		if ($(this).hasClass('btn-default')) {
			...
			{	
		  	...
		  	//		  	$(this).html('<i class="fa fa-spinner fa-spin"></i> <@messages key="common.op.doing" />').parent().find('button').prop('disabled', true);
						...
						var labels=''; 
						$('input[type="checkbox"][name="label"]').each(
						    function() 
						    {
						      if ($(this).is(":checked")) {
						      		labels =labels+$(this).val()+'&01,';
						    	}
						    	else
						    	{
						    		labels =labels+$(this).val()+'&00,';
						    	}
						    }
						);
						$('#labels').val(labels);  
